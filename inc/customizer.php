...
function pre_underscores_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'pre_underscores' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options',
		array(
			'title'			=> __( 'Theme Options', 'pre_underscores' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'pre_underscores' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'pre_underscores_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'pre_underscores_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'pre_underscores' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'pre_underscores' ),
				'full-content'	=> __( 'Full content', 'pre_underscores' )
			),
...
function pre_underscores_customize_preview_js() {
	wp_enqueue_script( 'pre_underscores_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
...
function pre_underscores_sanitize_length( $value ) {
...
if ( ! function_exists( 'pre_underscores_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see pre_underscores_custom_header_setup().
	 */
	function pre_underscores_header_style() {
